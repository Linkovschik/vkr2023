...
open class MapCongestionService(
    ...
    open fun calcCongestion(
        ...
        for (currentMinute in startTimeInMinutes..endTimeInMinutes step timeWindowInMinutes) {
            val filteredSquares = mapSquares
            ...
            if (filteredSquares.isEmpty())
                continue

            results.add(
                filteredSquares
                    .reduce { acc, congestion -> acc + congestion }
                    .divide(BigDecimal(filteredSquares.size), 4, RoundingMode.HALF_UP)
            )
        }

        if (results.isEmpty())
            return CongestionResult(BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO)
