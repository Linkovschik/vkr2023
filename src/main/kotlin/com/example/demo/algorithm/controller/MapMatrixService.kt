...
class MapMatrixService() {
    fun createMatrix(leftBotPoint: MapPoint = MapPoint(55.87409794116178, 54.668937265391214),
                     rightTopPoint: MapPoint = MapPoint(56.15974247241177, 54.86664699292725),
                     size: Int = 30,
        ...
        for (i in 0 until size) {
            for (j in 0 until size) {
                result.add(
                        MapSquare(
                                MapPoint(leftBotPoint.lng + lngStep * i, leftBotPoint.lat + latStep * j),
                                MapPoint(leftBotPoint.lng + lngStep * i, leftBotPoint.lat + latStep * (j + 1)),
                                MapPoint(leftBotPoint.lng + lngStep * (i + 1), leftBotPoint.lat + latStep * (j + 1)),
                                MapPoint(leftBotPoint.lng + lngStep * (i + 1), leftBotPoint.lat + latStep * j)
                        ))
            }
        }
